...
docker rm news_classification_pipeline 2> /dev/null || true


echo "Building docker image ..."
docker build -t news_classification_pipeline .  >/dev/null 2>&1 &&

echo " "
printf "\nRunning pipeline. This will take a few minutes.\n"
docker run --name news_classification_pipeline -it news_classification_pipeline &&
